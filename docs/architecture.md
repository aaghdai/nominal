# Nominal Architecture Overview

## System Architecture

```
┌─────────────────────────────────────────────────────────────┐
│                     NOMINAL SYSTEM                           │
└─────────────────────────────────────────────────────────────┘

┌──────────────┐       ┌──────────────┐       ┌──────────────┐
│   INPUT      │       │  PROCESSING  │       │   OUTPUT     │
│              │       │              │       │              │
│  PDF Files   │──────▶│  Components  │──────▶│  Organized   │
│  (Tax Forms) │       │              │       │  Files       │
└──────────────┘       └──────────────┘       └──────────────┘
```

## Component Architecture

```
┌─────────────────────────────────────────────────────────────┐
│ 1. NOMINAL READER (✅ Complete)                              │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  Input: PDF File                                            │
│     │                                                        │
│     ├─▶ Text Extraction (PyMuPDF)                           │
│     │      │                                                 │
│     │      ├─ Text Found? ─▶ Return Text                    │
│     │      │                                                 │
│     │      └─ Low/No Text? ─▶ OCR (Tesseract)               │
│     │                            │                           │
│     └────────────────────────────┘                           │
│                                                              │
│  Output: Text Content                                       │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│ 2. NOMINAL PROCESSOR (✅ Complete)                           │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  Input: Text Content + Rule Files                           │
│     │                                                        │
│     ├─▶ Load Rules (RuleParser)                             │
│     │      │                                                 │
│     │      └─▶ Parse YAML ─▶ Rule Objects                   │
│     │                                                        │
│     ├─▶ Match Criteria (CriteriaEvaluator)                  │
│     │      │                                                 │
│     │      ├─ Check: contains, regex, all, any              │
│     │      │                                                 │
│     │      └─ Match Found? ─┐                                │
│     │                       │                                │
│     │                       ├─ Yes ─▶ Execute Actions       │
│     │                       │                                │
│     │                       └─ No ─▶ Try Next Rule          │
│     │                                                        │
│     └─▶ Extract Variables (ActionExecutor)                  │
│            │                                                 │
│            └─▶ set, regex_extract, derive, extract          │
│                                                              │
│  Output: {form_name, variables}                             │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│ 3. NOMINAL ORCHESTRATOR (🔄 Planned)                         │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  Input: Directory of PDFs                                   │
│     │                                                        │
│     ├─▶ For each file:                                      │
│     │      │                                                 │
│     │      ├─▶ Read (Nominal Reader)                        │
│     │      │                                                 │
│     │      ├─▶ Process (Nominal Processor)                  │
│     │      │                                                 │
│     │      ├─▶ Generate Filename                            │
│     │      │      │                                          │
│     │      │      └─▶ Template: {FORM}_{NAME}_{SSN}        │
│     │      │                                                 │
│     │      └─▶ Copy/Rename to Output Directory              │
│     │                                                        │
│     └─▶ Error Handling & Logging                            │
│                                                              │
│  Output: Organized Files + Logs                             │
└─────────────────────────────────────────────────────────────┘
```

## Data Flow

```
┌────────────┐
│ Input.pdf  │
└─────┬──────┘
      │
      ▼
┌─────────────────┐
│ Nominal Reader  │
└─────┬───────────┘
      │
      ▼ (text content)
      │
      │   ┌──────────────┐
      │   │ Rule Files   │
      │   │  - w2.yaml   │
      │   │  - 1099.yaml │
      │   └──────┬───────┘
      │          │
      ▼          ▼
┌──────────────────────┐
│ Nominal Processor    │
│                      │
│  ┌────────────────┐  │
│  │ Match Criteria │  │
│  └────────┬───────┘  │
│           │          │
│           ▼          │
│  ┌────────────────┐  │
│  │ Execute Actions│  │
│  └────────┬───────┘  │
└───────────┼──────────┘
            │
            ▼
┌───────────────────────┐
│ Extracted Variables   │
│                       │
│ {                     │
│   form_name: "W2"     │
│   FIRST_NAME: "John"  │
│   LAST_NAME: "Smith"  │
│   SSN: "XXX-XX-6789"  │
│   SSN_LAST_FOUR: "..."│
│ }                     │
└───────────┬───────────┘
            │
            ▼
┌──────────────────────────┐
│ Output: W2_Smith_6789.pdf│
└──────────────────────────┘
```

## Rule Processing Flow

```
┌─────────────┐
│ Document    │
│ Text        │
└──────┬──────┘
       │
       ▼
┌──────────────────┐
│ For Each Rule:   │
└──────┬───────────┘
       │
       ▼
┌────────────────────────┐
│ Evaluate Criteria      │
│                        │
│  ├─ contains          │
│  │   "w-2"?           │
│  │   ✓ Yes            │
│  │                    │
│  ├─ regex             │
│  │   \d{3}-\d{2}-\d{4}│
│  │   ✓ Match: captured│
│  │                    │
│  └─ any/all           │
│      ✓ All Match      │
└────────┬───────────────┘
         │
         ▼ (match found)
┌────────────────────────┐
│ Execute Actions        │
│                        │
│  1. set: FORM_NAME     │
│     FORM_NAME = "W2"   │
│                        │
│  2. regex_extract:     │
│     FIRST_NAME = "..."│
│                        │
│  3. derive:            │
│     SSN_LAST_FOUR = ...│
└────────┬───────────────┘
         │
         ▼
┌────────────────────────┐
│ Return Result          │
│ {form_name, variables} │
└────────────────────────┘
```

## Class Hierarchy

```
NominalReader
├─ __init__(ocr_fallback, min_text_length)
├─ read_pdf(file_path) → str
└─ _ocr_page(page) → str

NominalProcessor
├─ __init__(rules_dir)
├─ load_rules(rules_dir)
├─ load_rule(rule_path)
├─ process_document(text) → Dict
└─ _try_match_rule(rule, text) → Dict

RuleParser
├─ parse_file(rule_path) → Rule
├─ parse_dict(data) → Rule
├─ _parse_criterion(data) → Criterion
└─ _parse_action(data) → Action

CriteriaEvaluator
├─ evaluate(criterion, text) → bool
├─ _evaluate_contains(criterion, text) → bool
└─ _evaluate_regex(criterion, text) → bool

ActionExecutor
├─ execute(action, captured_values)
├─ _execute_set(action)
├─ _execute_regex_extract(action)
├─ _execute_derive(action)
└─ _execute_extract(action)
```

## File Organization

```
nominal/
│
├── src/nominal/
│   ├── __init__.py
│   ├── reader.py           ← Milestone 1 ✅
│   └── processor.py        ← Milestone 2 ✅
│
├── rules/                  ← Rule Definitions
│   ├── w2.yaml
│   ├── 1099-misc.yaml
│   └── README.md
│
├── test/
│   ├── fixtures/           ← Sample PDFs
│   │   ├── Sample-W2.pdf
│   │   └── Sample-1099-image.pdf
│   │
│   └── nominal/
│       ├── test_reader.py
│       ├── test_reader_integration.py
│       ├── test_processor.py            ← 19 tests
│       └── test_processor_integration.py ← 5 tests
│
├── docs/
│   ├── processor.md
│   └── milestone2_summary.md
│
├── example_processor.py
├── PLAN.md
└── README.md
```

## Technology Stack

```
┌─────────────────────────────────────┐
│           Python 3.13+              │
└─────────────────────────────────────┘
              │
      ┌───────┴────────┐
      │                │
┌─────▼─────┐   ┌──────▼──────┐
│  Reader   │   │  Processor  │
└───────────┘   └─────────────┘
      │               │
      │               │
┌─────▼─────┐   ┌─────▼─────┐
│ PyMuPDF   │   │  PyYAML   │
│ Tesseract │   │  re       │
│ Pillow    │   │ (stdlib)  │
└───────────┘   └───────────┘
```

## Key Features by Milestone

### ✅ Milestone 1: Reader
- PDF text extraction
- OCR for image-based PDFs
- Configurable thresholds
- Error handling

### ✅ Milestone 2: Processor
- YAML-based DSL
- Pattern matching (contains, regex, all, any)
- Variable extraction (set, regex_extract, derive, extract)
- Batch processing
- First-match rule selection
- 29 tests, all passing

### 🔄 Milestone 3: Orchestrator (Next)
- Directory scanning
- File renaming
- Error logging
- CLI interface
- Configuration management

