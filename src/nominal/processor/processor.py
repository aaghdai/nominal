...
class NominalProcessor:
    ...
    def process_document(self, text: str, enforce_global: bool = False) -> Optional[Dict[str, Any]]:
        """
        ...
        for rule in self.rules:
            ...
            if result:
                    ...
                    for var_name, var_value in global_vars.items():
                        if var_name in self.global_variables:
                            if self.global_variables[var_name] != var_value:
                                conflicts.append(f"{var_name}: '{self.global_variables[var_name]}' vs '{var_value}'")
                    
                    if conflicts:
                        logger.warning(
                            f"Global variable conflict detected, skipping document. Conflicts: {', '.join(conflicts)}"
                        )
                        continue
                ...
                logger.info(f"  Global: {len(global_vars)} var(s), Local: {len(local_vars)} var(s), Derived: {len(derived_vars)} var(s)")
    ...
    def process_batch(self, documents: List[str], enforce_global: bool = True) -> List[Optional[Dict[str, Any]]]:
        """
        ...
        logger.info(f"Batch processing complete: {matched_count}/{len(documents)} documents matched")
