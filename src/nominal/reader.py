        ...
        try:
            ...
            for page_num, page in enumerate(doc, 1):
                ...
                if len(text.strip()) < self.min_text_length:
                    ...
                    if page.get_images():
                        should_ocr = True
                        logger.debug(f"Page {page_num}: Text too sparse ({len(text.strip())} chars), attempting OCR")
                ...
                if not should_ocr and self.ocr_fallback:
                    ...
                    for img in images_info:
                        ...
                        if img_area > (page_area * 0.3):
                            should_ocr = True
                            logger.debug(f"Page {page_num}: Large image detected ({img_area/page_area*100:.1f}% of page), attempting OCR")
                            break
                ...
                if self.ocr_fallback and should_ocr:
                    ...
                    if len(ocr_text.strip()) > len(text.strip()) * 1.2:
                        text = ocr_text
                        logger.info(f"Page {page_num}: OCR provided {len(ocr_text)} chars (vs {len(page.get_text())} from text extraction)")
                    else:
                        logger.debug(f"Page {page_num}: OCR did not provide enough improvement, using text extraction")
                
                text_content.append(text)
                
            doc.close()
            total_text = len("\n".join(text_content))
            logger.info(f"Successfully read PDF: {total_text} total characters from {total_pages} page(s)")
